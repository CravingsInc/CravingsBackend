...
router.get('/all/incomplete', async ( req, res ) => {
    try {
        ...
        let incompleteReviews = await models.EventTicketCart.query(`
            select etc.id, etc.stripeTransactionId, etc.eventId, etc.type, etc.lastReviewEmailSent, etc.userId, etc.reviewId, etc.name, etc.email, e.title, e.banner from event_ticket_cart etc
